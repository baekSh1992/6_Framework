...
@Controller
@SessionAttributes({"loginMember", "message", "test1"}) // 배열 형식으로쓰고 싶으면 ({"loginMember", "test1", "test2"})
...
public class MemberController {
	...
	// @Autowired
	// baen scanning을 통해 bean으로 등록된 객체 중 
	...
	public String login(HttpServletRequest req) {
		...
		// * forward  방식 : prefix/suffix를 제외한 나머지 jsp 경로

		// * redirect 방식 : "redirect:요청주소";
		return "redirect:/";
	}
...
//	@RequestMapping(value="/member/login", method = RequestMethod.POST)
    ...
    // --> Spring Container(SPring관련된 모든 것이 담겨있는 박스)에서
    //     Argument Resolver 제공(매개변수 해결사)해서 유연하게 해결함
		...
		// 로그인 실패 시 "아이디 또는 비밀번호가 일치하지 않습니다" 세션에 추가
		 ...
		 if(loginMember != null) {
			 ...
			 if(saveId != null) { // 체크 되었을 때
				 ...
				 // --> 클라이언트의 쿠키 파일을 삭제 
				 cookie.setMaxAge(0); // 쿠키 생성과 동시에 삭제
			 ...
			 // 생성된 쿠키를 응답 객체 담아서 클라이언트에게 전달하는 
...
//			 model.addAttribute("message", "아이디 또는 비밀번호가 일치하지 않습니다.");
		...
		return "redirect:" + path; // 뷰 리저버로 안가고 다시 디스패쳐 서블릿이 재요청으로 인식해서 핸들러맵핑으로 가서 컨트롤러로 이동
	}
	...
	public String logout(SessionStatus status) {
		...
		// 왜,why -> @SessinAttribute로 session scope에 등록된 값을 무효화 시키려면
...
//	@ExceptionHandler(Exception.class) 어노테이션만 주석처리해도 밑에 코드가 동작 안

...
<body>
    <!-- 
        ...
        -> 그래서 #(id) 또는 .(class) 속성을 필수적으로 추가해야하는 번거로움이 있음

        이러한 문제점을 해결하고자
        태그의 이름만으로 어느정도 어떤 역할을 하는지 알 수 있고
        추가적으로 웹 접근성 향상에 도움이 되는
        시멘틱 태그 (Semantic Tag, 의미있는 태그)가 HTML5에서 추가됨.

        *시멘틱 태그는 div 태그의 이름만 바뀐 것으로 생각하는게 좋다!

        [시멘틱 태그 종류]
        <main>    : 현재 문서의 주된 콘텐츠를 작성하는 영역
        
        <header>  : 문서의 제목, 머리말 영역 외에도 위쪽에 있었으면 하는ㅇ ㅐ들 작성
        
        <footer>  : 문서의 하단 부분, 꼬리말, 정보 작성 영역 
                    (웹페이지 하단 고객센터 번호, 회사주소 등등 적는 부분)

        <nav>     : 나침반 역할 (다른 페이지, 사이트 이동)의 링크 작성 영역 
        (navigate줄임말, 네이버 메인에서 검색창 밑에 메일,카페,블로그 등 적힌 얇은부분)

        <aside>   : 사이드바, 광고 등을 표시하는 양쪽 영역

        <section> : 구역을 구분하기 위한 영역
        
        <article> : 본문과 독립된 콘텐츠를 작성하는 영역(웹 페이지 중간중간 광고 삽입 부분)
        -->
        ...
        <main>
            ...
            <%-- 
                jsp 액션 태그 중 include
                - 해당 위치에 page속성으로 지정된 jsp 파일의 내용이 포함됨
